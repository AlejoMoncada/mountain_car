"""Run one fresh, reproducible DQN experiment on MountainCar-v0."""
import argparse
import csv
import json
import platform
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eval-seed", type=int, default=30_250_310)
    parser.add_argument("--eval-episodes", type=int, default=100)
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/dqn/seed-20250310"))
...
def validate(args: argparse.Namespace) -> Path:
    ...
    save_path = args.save_path or Path(f"saves/dqn_seed-{args.seed}.pt")
    if save_path.exists():
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.open("w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=list(EPISODE_FIELDS))
        writer.writeheader()
        for row in rows:
            writer.writerow(row)


def evaluate(agent: DQNAgent, episodes: int, seed: int) -> list[dict[str, Any]]:
    env = gym.make(ENV_ID)
    rows = []
    try:
...
def main() -> None:
    ...
    summary = {
        ...
        "hyperparameters": {
            ...
            for name in DQNAgent._HPARAMS
            if name != "env_id"
        }
        | {"epsilon_start": 1.0},
        "versions": {name: version(name) for name in ("gymnasium", "matplotlib", "numpy", "torch", "mountain_car")}
